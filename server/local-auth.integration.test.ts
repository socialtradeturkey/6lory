...
import { localAuthCredentials, users } from "../drizzle/schema.js";
...
describeReal("local auth gerçek DB akışı", () => {
  it("kayıt, cookie session, giriş ve duplicate kayıt korumasını tamamlar", async () => {
    ...
    try {
      ...
      await expect(caller.auth.register({ name: "Integration Auth", email, password })).resolves.toEqual({ success: true });
      expect(cookies).toHaveLength(1);
      expect(cookies[0]?.value).toBeTruthy();

      await expect(caller.auth.login({ email, password })).resolves.toEqual({ success: true });
      await expect(caller.auth.login({ email, password: "YanlisParola123" })).rejects.toMatchObject({ code: "UNAUTHORIZED" });
      await expect(caller.auth.register({ name: "Duplicate", email: email.toUpperCase(), password })).rejects.toMatchObject({ code: "CONFLICT" });
