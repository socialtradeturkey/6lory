      ...
      <div className="mx-auto flex min-h-dvh max-w-[1440px]">
        <aside className="sticky top-0 hidden h-dvh w-[252px] shrink-0 flex-col border-r border-border/70 bg-background/70 px-4 py-6 backdrop-blur-xl md:flex">
          ...
          <div className="mt-auto space-y-3 rounded-2xl border border-border/80 bg-card/65 p-3 shadow-sm">
            ...
            {isAuthenticated ? <Button onClick={() => logout()} variant="ghost" className="w-full justify-start gap-2 text-muted-foreground"><LogOut className="size-4" /> Çıkış yap</Button> : <Button onClick={() => startLogin()} className="w-full" title="Yeni hesaplar ilk Google/Manus girişinde güvenli biçimde oluşturulur">Google ile devam et</Button>}
          </div>
        </aside>
